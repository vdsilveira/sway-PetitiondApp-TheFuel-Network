...
var collectBinFilepaths = (params) => {
  ...
  const binFiles = filepaths.map((abiFilepath) => {
    ...
    const bin = {
      path: binFilepath,
      contents: (0, import_bytes.hexlify)((0, import_fs.readFileSync)(binFilepath))
    };
    return bin;
  });
  return binFiles;
...
function configureCliOptions(program) {
  program.requiredOption("-i, --inputs <path|glob...>", "input paths/globals to your abi json files").requiredOption("-o, --output <dir>", "directory path for generated files").addOption(
