...
__export(util_exports, {
  ...
  includeHexPrefix: () => includeHexPrefix
});
...
var getContractRoot = (bytecode) => {
  ...
  const bytes = (0, import_bytes.arrayify)(bytecode);
  const chunks = (0, import_utils.chunkAndPadBytes)(bytes, chunkSize);
  return (0, import_merkle.calcRoot)(chunks.map((c) => (0, import_bytes.hexlify)(c)));
};
var getContractStorageRoot = (storageSlots) => {
  const tree = new import_merkle.SparseMerkleTree();
  storageSlots.forEach(({ key, value }) => tree.update((0, import_sha2.sha256)(key), value));
  return tree.root;
};
var getContractId = (bytecode, salt, stateRoot) => {
  const root = getContractRoot((0, import_bytes.arrayify)(bytecode));
  const contractId = (0, import_sha2.sha256)((0, import_bytes.concat)(["0x4655454C", salt, root, stateRoot]));
  return contractId;
};
var includeHexPrefix = (value, options) => (0, import_bytes.hexlify)(value, {
  ...options,
...
var ContractFactory = class {
  ...
  constructor(bytecode, abi, accountOrProvider = null) {
    this.bytecode = (0, import_bytes2.arrayify)(bytecode);
  ...
  setConfigurableConstants(configurableConstants) {
    try {
      ...
      Object.entries(configurableConstants).forEach(([key, value]) => {
        ...
        const bytes = (0, import_bytes2.arrayify)(this.bytecode);
