...
import { hexDataSlice, concat, hexlify, arrayify as arrayify2 } from "@ethersproject/bytes";
import { pbkdf2 } from "@ethersproject/pbkdf2";
import { computeHmac, sha256 as sha2562, SupportedAlgorithm } from "@ethersproject/sha2";
...
var Mnemonic = class {
  ...
  static entropyToMnemonic(entropy, wordlist = english) {
    const entropyBytes = arrayify2(entropy, {
  ...
  static masterKeysFromSeed(seed) {
    ...
    return arrayify2(computeHmac(SupportedAlgorithm.sha512, MasterSecret, seedArray));
  }
  /**
  ...
  static seedToExtendedKey(seed, testnet = false) {
    ...
    const prefix = arrayify2(testnet ? TestnetPRV : MainnetPRV);
    const depth = "0x00";
    ...
    const checksum = hexDataSlice(sha2562(sha2562(extendedKey)), 0, 4);
  ...
  static generate(size = 32, extraEntropy = "") {
    const entropy = extraEntropy ? sha2562(concat([randomBytes(size), arrayify2(extraEntropy)])) : randomBytes(size);
