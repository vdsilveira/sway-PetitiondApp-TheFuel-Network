...
var InstructionSet = class {
  ...
  toHex() {
    return (0, import_bytes.hexlify)(this.toBytes());
  }
  toString() {
    return `Program:
...
var SCRIPT_DATA_BASE_OFFSET = import_abi_coder.VM_TX_MEMORY + import_abi_coder.SCRIPT_FIXED_SIZE;
...
var ScriptRequest = class {
  ...
  constructor(bytes, scriptDataEncoder, scriptResultDecoder2) {
    this.bytes = (0, import_bytes2.arrayify)(bytes);
  ...
  encodeScriptData(data) {
    ...
    this.bytes = (0, import_bytes2.arrayify)(callScript.script);
...
var scriptResultDecoder = (contractId, isOutputDataHeap) => (result) => {
  ...
  return receipts.filter(({ type }) => isReturnType(type)).flatMap((receipt, index, filtered) => {
    ...
    if (receipt.type === import_transactions4.ReceiptType.ReturnData) {
      const encodedScriptReturn = (0, import_bytes3.arrayify)(receipt.data);
      if (isOutputDataHeap && isReturnType(filtered[index + 1]?.type)) {
        const nextReturnData = filtered[index + 1];
        return (0, import_bytes3.concat)([encodedScriptReturn, (0, import_bytes3.arrayify)(nextReturnData.data)]);
      }
...
var getContractCallScript = (functionScopes) => new ScriptRequest(
  ...
  (contractCalls) => {
    ...
    for (let i = 0; i < TOTAL_CALLS; i += 1) {
      ...
      const args = (0, import_bytes3.arrayify)(call2.data);
      scriptData.push(args);
      segmentOffset = dataOffset + (0, import_bytes3.concat)(scriptData).byteLength;
    }
    const script = getInstructions(paramOffsets, outputInfos);
    const finalScriptData = (0, import_bytes3.concat)(scriptData);
...
var BaseInvocationScope = class {
  ...
  get calls() {
    ...
    return this.functionInvocationScopes.map(
      (funcScope) => createContractCall(funcScope, script.getScriptDataOffset(maxInputs))
    );
  }
  /**
  ...
  getRequiredCoins() {
    ...
    })).concat(this.transactionRequest.calculateFee(gasPriceFactor)).filter(({ assetId, amount }) => assetId && !(0, import_math3.bn)(amount).isZero());
    return assets;
  }
  /**
  ...
  async getTransactionCost(options) {
    ...
    const request = (0, import_providers2.transactionRequestify)(this.transactionRequest);
  ...
  async dryRun() {
    const provider = this.getProvider();
    const transactionRequest = await this.getTransactionRequest();
    const request = (0, import_providers2.transactionRequestify)(transactionRequest);
